...
ROOT_ADDR=$($TONOS_CLI -u $NETWORK genaddr ./src/20_bomber.tvc ./src/20_bomber.abi.json --genkey $CURRENT_DIR/bomber/deploy.keys.json --wc 0 | grep "Raw address:" | cut -c 14-)
...
echo { \"address\": \"$ROOT_ADDR\"} > $CURRENT_DIR/bomber/address.json
echo bomber: $ROOT_ADDR
...
RESULT_DEPLOY=$($TONOS_CLI -u $NETWORK deploy ./src/20_bomber.tvc {} --abi ./src/20_bomber.abi.json --sign ./bomber/deploy.keys.json --wc 0 | grep "Transaction" | cut -c 12-) #| awk '/Transaction /')
