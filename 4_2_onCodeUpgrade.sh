...
echo ==========================================================================
# CODE=$($TVM_LINKER decode --tvc ./src/NewVersion.tvc  | grep code: | cut -c 8-)

# echo $CODE
ADDR_FILE="./BadContract/address.json"
...
# RESULT_DEPLOY=$(
    # $TONOS_CLI -u $NETWORK call $ADDR setCode $ROOT_DATA --abi ./src/BadContract.abi.json --sign ./BadContract/deploy.keys.json #| grep "Transaction" | cut -c 12-) 
...
$TONOS_CLI -u $NETWORK run $ADDR version {} --abi ./src/NewVersion2.abi.json | awk '/Result: {/,/}/'

# $TONOS_CLI -u $NETWORK run $ADDR value1 {} --abi ./src/NewVersion2.abi.json | awk '/Result: {/,/}/'
